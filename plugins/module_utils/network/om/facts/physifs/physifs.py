...
class PhysifsFacts(object):
    ...
    def render_config(self, spec, conf):
        ...
        for option in config.keys():
            if option in conf:
                if isinstance(config[option], dict):
                    for suboption in config[option].keys():
                        if suboption in conf[option]:
                            config[option][suboption] = conf[option][suboption]
                else:
